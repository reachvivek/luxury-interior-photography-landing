...
async function getSubcategoryData() {
  try {
    ...
    return (record?.data as any)?.hospitality?.subcategories?.restaurants ?? null;
  } catch {
    return null;
  }
}
...
const FALLBACK = {
  ...
  description:
    "Stunning restaurant photography that captures the ambiance, design, and culinary atmosphere of fine dining establishments and casual eateries.",
  ...
  impactDescription:
    "Our restaurant photography transforms marketing materials into powerful booking drivers. Restaurants featuring MyVisual.Space's imagery see 60-80% increases in social media engagement, with images regularly shared by diners and influencers. More importantly, these visuals translate to measurable business results - 25-40% increases in online reservations and enhanced ability to command premium pricing for special events and private bookings.",
  galleryImages: [
    {
...
export default async function RestaurantsPage() {
  ...
  const videos = await getServiceVideos("hospitality/restaurants");
