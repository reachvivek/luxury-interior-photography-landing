...
async function getSubcategoryData() {
  try {
    ...
    return (record?.data as any)?.commercial?.subcategories?.["coworking-spaces"] ?? null;
  } catch {
    return null;
  }
}
...
const FALLBACK = {
  ...
  description:
    "Dynamic photography of collaborative co-working environments, hot desks, private booths, and community spaces that inspire creativity and connection.",
  ...
  impactDescription:
    "Our coworking space photography directly impacts occupancy rates and membership conversion. Spaces featuring MyVisual.Space's imagery consistently see 50-70% increases in virtual tour engagement and 40% higher conversion rates from inquiry to membership. By visually communicating the community vibe, amenity quality, and professional environment, we help operators attract the right members and justify premium pricing in competitive markets.",
  galleryImages: [
    {
...
export default async function CoworkingSpacesPage() {
  ...
  const videos = await getServiceVideos("commercial/coworking-spaces");
