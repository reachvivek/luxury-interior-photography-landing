...
async function getSubcategoryData() {
  try {
    ...
    return (record?.data as any)?.hospitality?.subcategories?.resorts ?? null;
  } catch {
    return null;
  }
}
...
const FALLBACK = {
  ...
  description:
    "Breathtaking resort photography capturing the essence of luxury destinations, from beachfront paradises to mountain retreats, showcasing both grand amenities and intimate guest experiences.",
  ...
  impactDescription:
    "Professional resort photography directly impacts revenue by reducing dependency on OTAs, supporting premium pricing strategies, and driving direct bookings. Resorts featuring MyVisual.Space's imagery report 45-60% increases in direct website bookings, 30% higher conversion rates, and the ability to command 15-20% rate premiums. Our photography becomes the cornerstone of integrated marketing campaigns across social media, print advertising, and partnership materials that establish your resort as a bucket-list destination.",
  galleryImages: [
    {
...
export default async function ResortsPage() {
  ...
  const videos = await getServiceVideos("hospitality/resorts");
