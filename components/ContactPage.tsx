...
export default function ContactPage({ contactData }: ContactPageProps) {
  const data = contactData ?? contactDefaults;
      ...
      <section className="pt-32 pb-16 px-6 md:px-16 bg-white">
        <div className="max-w-5xl mx-auto text-center">
          <h1 className="text-5xl md:text-6xl lg:text-7xl font-serif font-light text-stone-900 mb-6">
            {data.hero?.title ?? contactDefaults.hero.title}
          </h1>
          <p className="text-xl text-stone-600 max-w-3xl mx-auto leading-relaxed">
            {data.hero?.subtitle ?? contactDefaults.hero.subtitle}
          </p>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-16">
          ...
          <div>
            <h2 className="text-3xl font-serif font-light text-stone-900 mb-8">
              {data.form?.title ?? contactDefaults.form.title}
            </h2>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                >
                  ...
                  {(data.form?.projectTypes ?? contactDefaults.form.projectTypes).map(
                    (type: string) => (
          ...
          <div className="space-y-12">
            <div>
              <h2 className="text-3xl font-serif font-light text-stone-900 mb-8">
                {data.info?.title ?? contactDefaults.info.title}
              </h2>
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-lg text-stone-600">
                    {data.info?.location ?? contactDefaults.info.location}
                  </p>
                </div>
                <div>
                  <h3 className="text-sm font-medium text-stone-700 tracking-wider uppercase mb-2">
                  ...
                  >
                    +{data.info?.whatsappNumber ?? contactDefaults.info.whatsappNumber}
                  </a>
                </div>
                <div>
                  <h3 className="text-sm font-medium text-stone-700 tracking-wider uppercase mb-2">
            ...
            <div className="bg-[#F5F0ED] p-8 rounded-lg">
              <h3 className="text-2xl font-serif font-light text-stone-900 mb-4">
                {data.hours?.title ?? contactDefaults.hours.title}
              </h3>
              <div className="space-y-2 text-stone-600">
                {(data.hours?.schedule ?? contactDefaults.hours.schedule).map(
