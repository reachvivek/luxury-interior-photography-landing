...
async function getSubcategoryData() {
  ...
  return (data as any)?.hospitality?.subcategories?.["event-spaces"] ?? null;
}
...
const FALLBACK = {
  ...
  impactDescription:
    "Our event space photography directly impacts venue booking rates by helping planners envision their perfect event. Venues showcasing MyVisual.Space's imagery report 50-65% increases in qualified inquiries and 35% higher conversion rates from site visits to bookings. By presenting spaces that inspire emotional connections and practical confidence, we help venues command premium pricing and secure high-value corporate contracts and destination weddings.",
  galleryImages: [
    {
...
export default async function EventSpacesPage() {
  ...
  const videos = await getServiceVideos("hospitality/event-spaces");
