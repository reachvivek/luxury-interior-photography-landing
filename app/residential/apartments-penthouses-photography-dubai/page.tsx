...
async function getSubcategoryData() {
  try {
    ...
    return (record?.data as any)?.residential?.subcategories?.["apartments-penthouses"] ?? null;
  } catch {
    return null;
  }
}
...
const FALLBACK = {
  ...
  description:
    "Professional photography showcasing modern apartment living from cozy studios to luxury penthouses with panoramic city views, capturing sophisticated design and premium finishes.",
...
export default async function ApartmentsPenthousesPage() {
  const data = await getSubcategoryData();
  const d = data ?? FALLBACK;
  const videos = await getServiceVideos("residential/apartments-penthouses");
