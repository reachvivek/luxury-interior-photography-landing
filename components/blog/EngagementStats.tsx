"use client";

interface EngagementStatsProps {
  views: number;
  likes: number;
  commentCount: number;
  variant?: "compact" | "full";
  theme?: "light" | "dark";
  liked?: boolean;
  onLike?: () => void;
}

export default function EngagementStats({
  views,
  likes,
  commentCount,
  variant = "compact",
  theme = "light",
  liked = false,
  onLike,
}: EngagementStatsProps) {
  const formatNumber = (num: number): string => {
    if (num >= 1000) {
      return `${(num / 1000).toFixed(1)}k`;
    }
    return num.toString();
  };

  const textColor = theme === "dark" ? "text-white/80" : "text-stone-500";

  const heartClass = liked
    ? theme === "dark"
      ? "text-red-400 fill-red-400"
      : "text-red-500 fill-red-500"
    : "";

  const HeartButton = ({ size }: { size: string }) => {
    const sizeClass = size === "lg" ? "w-5 h-5" : "w-4 h-4";
    return (
      <button
        onClick={(e) => {
          e.preventDefault();
          e.stopPropagation();
          onLike?.();
        }}
        className={`inline-flex items-center gap-1.5 transition-transform active:scale-125 ${
          onLike ? "cursor-pointer hover:scale-110" : "cursor-default"
        }`}
      >
        <svg
          className={`${sizeClass} transition-all duration-200 ${heartClass}`}
          fill={liked ? "currentColor" : "none"}
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            strokeWidth={2}
            d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"
          />
        </svg>
      </button>
    );
  };

  if (variant === "compact") {
    return (
      <div className={`flex items-center gap-4 text-xs ${textColor}`}>
        <div className="flex items-center gap-1.5">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
          </svg>
          <span>{formatNumber(views)}</span>
        </div>
        <div className="flex items-center gap-1.5">
          <HeartButton size="sm" />
          <span>{formatNumber(likes)}</span>
        </div>
        <div className="flex items-center gap-1.5">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z" />
          </svg>
          <span>{commentCount}</span>
        </div>
      </div>
    );
  }

  const fullTextColor = theme === "dark" ? "text-white" : "text-stone-600";
  const fullLabelColor = theme === "dark" ? "text-white/70" : "text-stone-500";

  return (
    <div className={`flex items-center gap-6 md:gap-8 text-sm ${fullTextColor}`}>
      <div className="flex items-center gap-2">
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
        </svg>
        <span className="font-medium">{views.toLocaleString()}</span>
        <span className={fullLabelColor}>views</span>
      </div>
      <div className="flex items-center gap-2">
        <HeartButton size="lg" />
        <span className="font-medium">{likes.toLocaleString()}</span>
        <span className={fullLabelColor}>likes</span>
      </div>
      <div className="flex items-center gap-2">
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z" />
        </svg>
        <span className="font-medium">{commentCount}</span>
        <span className={fullLabelColor}>{commentCount === 1 ? "comment" : "comments"}</span>
      </div>
    </div>
  );
}
