...
async function getSubcategoryData() {
  try {
    ...
    return (record?.data as any)?.["custom-interiors"]?.subcategories?.["architectural-elements"] ?? null;
  } catch {
    return null;
  }
}
...
const FALLBACK = {
  ...
  impactDescription: "Our architectural element photography elevates design portfolios and marketing materials, helping architects and craftspeople showcase their expertise to high-value clients. These images become powerful sales tools - demonstrating technical capability, design innovation, and attention to quality that justifies premium pricing. Clients report that MyVisual.Space's detailed architectural photography helps them win 30-45% more bespoke commissions by clearly communicating their mastery of complex design challenges.",
  galleryImages: [
    {
...
export default async function ArchitecturalElementsPage() {
  const data = await getSubcategoryData();
  const d = data ?? FALLBACK;
  const videos = await getServiceVideos("custom-interiors/architectural-elements");
