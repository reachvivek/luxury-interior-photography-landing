...
async function getSubcategoryData() {
  try {
    ...
    return (record?.data as any)?.hospitality?.subcategories?.["hotel-suites"] ?? null;
  } catch {
    return null;
  }
}
...
export default async function HotelSuitesPage() {
  const data = await getSubcategoryData();
  const d = data ?? FALLBACK;
  const videos = await getServiceVideos("hospitality/hotel-suites");
